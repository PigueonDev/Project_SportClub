...
const NAV_ITEMS = [
  { key: 'inicio', label: '🏠 Inicio', path: '/user/dashboard' },
  { key: 'clases', label: '🏃 Clases Disponibles', path: '/user/clases' },
  { key: 'reservas', label: '📅 Mis Reservas', path: '/user/mis-reservas' },
];
    ...
    <Col md={2} className="d-flex flex-column p-3 shadow" style={{ backgroundColor: '#2E1A47' }}>
      ...
      <Nav className="flex-column mb-auto gap-1">
        {NAV_ITEMS.map(item => (
          <Button
            key={item.key}
            variant={active === item.key ? 'primary' : 'link'}
            ...
            style={active === item.key ? { backgroundColor: 'rgba(59, 130, 246, 0.2)' } : undefined}
          >
            {item.label}
          </Button>
        ))}
      ...
      <div className="mt-auto pt-3 border-top border-secondary d-flex flex-column gap-2">
        <Button variant="link" className="text-start text-warning text-decoration-none" onClick={() => navigate("/user/perfil")}>✏️ Editar perfil</Button>
